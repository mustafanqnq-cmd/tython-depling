...
# 1. سحب التوكنات والرموز من متغيرات البيئة لحمايتها
BOT_TOKEN = os.environ.get("BOT_TOKEN")
RAILWAY_API_KEY = os.environ.get("RAILWAY_API_KEY")
API_ID = 7219208
API_HASH = "64342b78a8d90e3f691d7a3a09112e7b"

# 2. قاعدة بيانات محلية خفيفة جداً (تحفظ الآيدي الخاص بالمستخدم وآيدي مشروعه فقط!)
...
def deploy_to_railway(session_string, user_id):
    ...
    env_variables = {
        ...
        "LAUNCHER_SECRET": "SUPHE999",  # سر الأنشر الأساسي
        "TZ": "Asia/Baghdad",
        "SESSION": session_string       # سيشن المستخدم (يحقن مباشرة في السيرفر)
    }
    ...
    try:
        ...
        # الخطوة 2: ربط ريبو الأنشر الخاص بك بالمشروع
...
async def deploy_handler(event):
    ...
    # التحقق مما إذا كان الآيدي مخزناً مسبقاً
    ...
    msg = await event.reply("⏳ جاري إنشاء البيئة وحقن الـ LAUNCHER_SECRET والفارات... يرجى الانتظار.")

    # تنفييذ عملية التنصيب وحقن الفارات
    success, result = deploy_to_railway(session_string, user_id)

    if success:
        project_id = result
        # نكتفي بحفظ الآيدي الخاص بالمستخدم ومعرف المشورع فقط
